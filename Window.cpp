...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "Practica 04: MODELADO JERARQUICO DANNAE SANCHEZ DURAN", NULL, NULL);
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	//AQUI EMPIEZAN LAS ARTICULACIONES DE LA GALLINA
	if (key == GLFW_KEY_B)
	{
		theWindow->rodillaizq += 10.0;
	}
	if (key == GLFW_KEY_N)
	{
		theWindow->rodilladerec += 10.0;
	}
	if (key == GLFW_KEY_M)
	{
		theWindow->pico += 10.0;
	...
	if (key == GLFW_KEY_V)
	{
		theWindow->alitaderec += 10.0;
	}
	if (key == GLFW_KEY_C)
	{
		theWindow->alitaizq += 10.0;
	}
